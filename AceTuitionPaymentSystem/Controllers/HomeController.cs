...
namespace AceTuitionPaymentSystem.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            int Dmonth = Convert.ToInt32(D_month);
            IEnumerable<ChildViewModel> studentList = (from objStudent in objAceTuitionEntities.tb_child
                                                         where objStudent.child_status=="active"
                                                         select new ChildViewModel()
                                                         {
                                                             child_ic = objStudent.child_ic,
                                                             child_name_eng = objStudent.child_name_eng,
                                                             child_code = objStudent.child_code,
                                                             child_name_chinese = objStudent.child_name_chinese,
                                                             child_year=objStudent.child_year
            ...
            IEnumerable<PaymentViewModel> pendingList = (from objPayment in objAceTuitionEntities.tb_payment
            ...
            IEnumerable<PaymentViewModel> totalPaymentList = (from objPayment in objAceTuitionEntities.tb_payment
                                                         where (objPayment.payment_month == Dmonth)
                                                         select new PaymentViewModel()
                                                         {
                                                             payment_date = objPayment.payment_date,
                                                             payment_month = objPayment.payment_month,
            ...
            IEnumerable<ReceiptViewModel> receiptList = (from objReceipt in objAceTuitionEntities.tb_receipt
                                                            join objPayment in objAceTuitionEntities.tb_payment on objReceipt.payment_id equals objPayment.payment_id
                                                            where (objPayment.payment_status == "accepted"  && objReceipt.receipt_status == "accepted")
            ...
            var mixmodel = new AdminListViewModel();

            mixmodel.childList = studentList;
            mixmodel.paymentList = outstandingList;
            mixmodel.receiptList = receiptList;
            mixmodel.pendingList = pendingList;
            mixmodel.totalPaymentList = totalPaymentList;
        ...
        public JsonResult FetchStudent()
        {
            using (var context = new AceTuitionEntities())
            {
                ...
                var data = context.tb_child.Where(x => x.child_status == "active").ToList();
                ...
                using (var Ep = new ExcelPackage())
                {
                    ...
                    foreach (var rec in receipt)
                    {
                        ...
                        if (item != null)
                        {
                            ...
                            paidAmount = paidAmount + item.payment_outstanding;
                            row++;
                            receiptCount++;
                        }
